...
function getPointGen() {
	...
	if (hasUpgrade('battery', 32) && hasUpgrade('battery', 21) && hasUpgrade('energy', 31)) {gain = gain.times(Decimal.max(1,player.battery.points.times(Decimal.max(1,player.battery.points.div(5))).pow(2)))}
	else if (hasUpgrade('battery', 32) && hasUpgrade('battery', 21)) {gain = gain.times(Decimal.max(1,player.battery.points.times(Decimal.max(1,player.battery.points.div(5)))))}
	else if (hasUpgrade('energy', 31) && hasUpgrade('battery', 21)) {gain = gain.times(Decimal.max(1,player.battery.points.pow(2))); }
	else if (hasUpgrade('battery', 21)) {gain = gain.times(Decimal.max(1,player.battery.points)); }
	...
	else if (hasUpgrade('darkenergy', 31)) {gain = new Decimal(0)}
	if (hasUpgrade('darkenergy', 32)) {gain = gain.times(10)}
	if (hasUpgrade('darkenergy', 42)) {gain = gain.times(10)}
	if (hasUpgrade('battery', 34)) {gain = gain.times(new Decimal(1).add(player.compactenergy.total.div(100)))}

	gain = Decimal.max(0, gain)
	return gain
}
...
let backgroundStyle = function(){ 
	if (player.achievements.doomsday.eq(1) && hasUpgrade('darkcore', 22) || player.bomb.total.gte(1)) {return {"background-image": "linear-gradient(rgb(53, 14, 14),rgb(0, 0, 0))"}}
